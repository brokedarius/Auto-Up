...
#Activate the 3 options below to run in headless mode, set screen resolution accordingly, headless mode is using a separate profile so be sure to log yourself the first time with the login script.
...
ForEach ($Item in $Args) {
    ...
    If (Test-Path -LiteralPath $Item -PathType Leaf) {
         ...
         #COVERS (if you need other formats or need to change suffixes, just add lines or change values in this next part)
     ...
     #This next line isn't mandatory, just special character replacement for the direct links, you can take it out if you never use it
     ...
     #CATEGORY (Must be on line 1 of your text file, no typos)

     $Category = GCI -LiteralPath "C:\Users\$Env:USERNAME\Desktop\Uploads\Presentations\$Name`_i.txt" | Get-content -TotalCount 1;

     $ChromeDriver.FindElementByXpath("/html/body/div[3]/div[2]/div/form/table/tbody/tr[2]/td[2]/select").SendKeys("$Category");
     ...
     #TAGS (Tags must be on line 3 of your text file, do not add line returns)
     ...
     #Closing tags for your bbcode (those you want after the screens, if not needed you can comment out this line)
     ...
     #DUPE CHECK (For now dupes will be logged no matter the % in the full auto version)
